    ...
    <main class="main_box container clearfix">
      <section class="topic_box clearfix">
        <ul class="classify clearfix">
          <li>
            <a href="#" @click="change(1)" :class="{ active: index == 1 }"
              >全部</a
            >
          </li>
          <li>
            <a href="#" @click="change(2)" :class="{ active: index == 2 }"
              >精华</a
            >
          </li>
          <li>
            <a href="#" @click="change(3)" :class="{ active: index == 3 }"
              >分享</a
            >
          </li>
          <li>
            <a href="#" @click="change(4)" :class="{ active: index == 4 }"
              >问答</a
            >
          </li>
          <li>
            <a href="#" @click="change(5)" :class="{ active: index == 5 }"
              >招聘</a
            >
          </li>
        </ul>
          ...
          <ul>
            <li v-for="(item, index) in topicData" :key="index">
              ...
              <router-link
                :to="{
                  path: 'detail',
                  query: { id: item.id, userId: item.author.loginname }
                }"
                class="title"
              >
              ...
              <span class="time">{{ T(item.create_at) }}</span>
            </li>
          </ul>
        </div>
        <!-- 分页 -->
          ...
          <ul>
            <li v-for="(hot, index) in getHot" :key="index">
              ...
              <router-link
                ...
                :to="{
                  path: 'detail',
                  query: { id: hot.id, userId: hot.author.loginname }
                }"
...
export default {
  ...
  methods: {
    handleSizeChange(val) {
      ...
      switch (this.index) {
        ...
        case 2:
          ...
          break;
        case 3:
          this.gettopicByTab(this.currentPage, this.limit, "share");
          break;
        case 4:
          this.gettopicByTab(this.currentPage, this.limit, "ask");
          break;
        case 5:
          this.gettopicByTab(this.currentPage, this.limit, "job");
          break;
      }
    },
    //当前页
    handleCurrentChange(val) {
      ...
      switch (this.index) {
        case 1:
          ...
          break;
        case 2:
          this.gettopicByTab(val, this.limit, "good");
          break;
        case 3:
          this.gettopicByTab(val, this.limit, "share");
          break;
        case 4:
          this.gettopicByTab(val, this.limit, "ask");
          break;
        case 5:
          this.gettopicByTab(val, this.limit, "job");
          break;
      }
    },
    //登录
    ...
    async gettopicByTab(page, limit, tab) {
      const { data, status } = await this.$http.get(
        `/topics?page=${page}&limit=${limit}&tab=${tab}`
      );
  ...
  mounted() {
    ...
    // console.log(time('2021-02-03T07:31:54.020Z'))
  },
  computed: {
    getHot() {
